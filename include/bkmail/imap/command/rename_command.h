/**
 * @file include/bkmail/imap/command/rename_command.h
 * @brief RENAME command.
...
///
/// Both mailbox arguments are rendered as astrings (atom when safe,
...
class rename_command {
 public:
  ...
  rename_command(std::string_view old_name, std::string_view new_name,
                 const Allocator& alloc = Allocator{})
      : old_name_(old_name, alloc), new_name_(new_name, alloc), wire_(alloc) {}
  ...
  void render(std::string_view tag) {
    wire_.assign(tag);
    wire_.append(" RENAME ");
    append_arg(old_name_);
    wire_.push_back(' ');
    append_arg(new_name_);
    wire_.append("\r\n");
  }
  ...
  /// RENAME never waits for a continuation request.
  ...
  /// RENAME expects no continuation; ignored.
  void on_continuation(std::string_view /*text*/) {}

  /// RENAME collects no untagged responses.
  void on_untagged(const untagged_response<Allocator>& /*r*/) {}

  /// Advisory for the write pump: RENAME pipelines freely.
 ...
 private:
  ...
  string_type old_name_;
  string_type new_name_;
...
#endif  // BKMAIL_IMAP_COMMAND_RENAME_COMMAND_H_
