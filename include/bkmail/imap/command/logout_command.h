/**
 * @file include/bkmail/imap/command/logout_command.h
...
#pragma once
#ifndef BKMAIL_IMAP_COMMAND_LOGOUT_COMMAND_H_
#define BKMAIL_IMAP_COMMAND_LOGOUT_COMMAND_H_
...
///
/// The server's untagged `BYE` that precedes the tagged reply is not
/// absorbed here; it flows through the unsolicited-event path.
template <class Allocator = std::allocator<std::byte>>
class logout_command {
 public:
  ...
  explicit logout_command(const Allocator& alloc = Allocator{})
      : wire_(alloc) {}
  ...
  /// LOGOUT never waits for a continuation request.
  ...
  /// LOGOUT expects no continuation; ignored.
  void on_continuation(std::string_view /*text*/) {}

  /// LOGOUT collects no untagged responses (the BYE is unsolicited).
  void on_untagged(const untagged_response<Allocator>& /*r*/) {}

  /// Advisory for the write pump: nothing meaningful follows LOGOUT.
...
#endif  // BKMAIL_IMAP_COMMAND_LOGOUT_COMMAND_H_
