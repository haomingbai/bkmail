/**
 * @file include/bkmail/imap/command/select_command.h
...
///
/// While in flight the command assembles the `mailbox_info` snapshot from
/// the untagged responses RFC 3501 mandates: `* n EXISTS`, `* n RECENT`,
/// `* FLAGS (...)`, and the `[UIDVALIDITY n]` / `[UIDNEXT n]` /
/// `[UNSEEN n]` / `[PERMANENTFLAGS (...)]` resp-codes of untagged `OK`s.
/// The tagged reply contributes `[READ-ONLY]` / `[READ-WRITE]` (default:
/// read-write). A tagged `NO` leaves the mailbox unselected: it maps to
/// `errc::command_rejected` and the result stays default-constructed.
///
...
class select_command {
 public:
  ...
  explicit select_command(std::string_view mailbox,
  ...
  /// SELECT never waits for a continuation request.
  ...
  /// SELECT expects no continuation; ignored.
  void on_continuation(std::string_view /*text*/) {}

  /// Absorbs the SELECT untagged set into the snapshot accumulators.
  ...
  /// Advisory for the write pump: SELECT pipelines freely.
  ...
  std::error_code on_tagged(const tagged_response<Allocator>& r,
    ...
    // SELECT OK defaults to READ-WRITE unless the code says otherwise.
    const bool read_only =
        r.code.has_value() && std::holds_alternative<read_only_code>(*r.code);
...
#endif  // BKMAIL_IMAP_COMMAND_SELECT_COMMAND_H_
