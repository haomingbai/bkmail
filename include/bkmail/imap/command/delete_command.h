/**
 * @file include/bkmail/imap/command/delete_command.h
...
#pragma once
#ifndef BKMAIL_IMAP_COMMAND_DELETE_COMMAND_H_
#define BKMAIL_IMAP_COMMAND_DELETE_COMMAND_H_
...
class delete_command {
 public:
  ...
  explicit delete_command(std::string_view mailbox,
  ...
  /// DELETE never waits for a continuation request.
  ...
  /// DELETE expects no continuation; ignored.
  void on_continuation(std::string_view /*text*/) {}

  /// DELETE collects no untagged responses.
  void on_untagged(const untagged_response<Allocator>& /*r*/) {}

  /// Advisory for the write pump: DELETE pipelines freely.
...
#endif  // BKMAIL_IMAP_COMMAND_DELETE_COMMAND_H_
